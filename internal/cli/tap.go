...
func newTapCmd(newRunner newRunnerFunc) *cobra.Command {
	...
	cmd.Flags().StringSlice("event-kind", nil, "event kinds to include: log|metric|trace (repeatable and/or comma-separated)")
	cmd.Flags().Bool("raw-format", false, "for gRPC API, output raw protojson event payload format")
